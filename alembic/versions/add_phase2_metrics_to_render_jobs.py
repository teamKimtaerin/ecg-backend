...
def upgrade():
    ...
    op.add_column('render_jobs', sa.Column('frames_processed', sa.Integer(), nullable=True))
    op.add_column('render_jobs', sa.Column('frames_dropped', sa.Integer(), nullable=True))
    ...
    op.add_column('render_jobs', sa.Column('memory_trend', sa.String(20), nullable=True))

    # Create indexes for performance queries
    op.create_index('idx_render_jobs_drop_rate', 'render_jobs', ['job_id', 'drop_rate'])
    op.create_index('idx_render_jobs_memory', 'render_jobs', ['job_id', 'memory_peak_mb'])
