...
async def trigger_gpu_server(job_id: str, request_data: Dict[str, Any], db_session: Session = None):
...
async def _send_request_to_gpu_server(job_id: str, payload: Dict[str, Any], db_session: Session = None) -> None:
    """GPU 서버에 렌더링 요청 전송"""
    try:
        # GPU 서버 요청 데이터 구성
        gpu_request = {
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(f"GPU 서버 요청 실패 - Job ID: {job_id}, Status: {response.status}, Error: {error_text}")
