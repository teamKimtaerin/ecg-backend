...
async def receive_ml_results(
    ...
    try:
        ...
        logger.info(f"ML 결과 수신 - Job ID: {job_id}, Status: {ml_result.status.value}, Progress: {ml_result.progress:.1%}")
        ...
        if ml_result.status == ProcessingStatus.STARTED:
            ...
            if ml_result.results and "message" in ml_result.results:
                job_status_store[job_id]["current_message"] = ml_result.results["message"]
                logger.info(f"진행 상황 - Job ID: {job_id}, Message: {ml_result.results['message']}")
...
async def trigger_ml_server(job_id: str, request: VideoProcessRequest):
    ...
    try:
        ...
        logger.info(f"ML API 호출 완료 - Job ID: {job_id}, 처리 시간: {result.get('processing_time', 0):.2f}초")
