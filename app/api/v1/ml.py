...
async def process_video(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"비디오 파일에 접근할 수 없습니다: {str(e)}"
            )
        ...
        ml_request = VideoProcessRequest(
            job_id=job_id,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"비디오 처리 요청 중 오류가 발생했습니다: {str(e)}"
        )
...
# 헬퍼 함수들
async def trigger_ml_server_processing(job_id: str, ml_request: VideoProcessRequest, db: Session):
    ...
    try:
        ...
        # ML 서버로 요청 전송
        await _send_request_to_ml_server(job_id, {
            "job_id": job_id,
            "video_url": ml_request.video_url
        })
