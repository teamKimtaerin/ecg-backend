...
try:
    ...
    print(
        "💡 Make sure to install LangChain packages: pip install langchain langchain-aws langchain-core langchain-community"
    )
    sys.exit(1)
...
def test_simple_prompt():
    ...
    try:
        ...
        print(
            f"   ⏱️  Processing: {result.get('request_params', {}).get('prompt_length')} chars input"
        )
...
def test_conversation_history():
    ...
    history = [
        ChatMessage(
            id="1", content="ECG가 무엇인가요?", sender="user", timestamp=datetime.now()
        ),
        ChatMessage(
            id="2",
...
def test_comparison():
    ...
    if results["original"] and results["langchain"]:
        print("\n📊 Comparison Results:")
        print(
            f"   📄 Original response preview: {results['original']['completion'][:100]}..."
        )
        print(
...
def main():
    ...
    try:
        ...
        print("📋 Test Summary:")
        print(
            f"   🔌 Basic Connection: {'✅ PASS' if (bedrock_ok and langchain_ok) else '❌ FAIL'}"
        )
        print(f"   💬 Simple Prompt: {'✅ PASS' if simple_ok else '❌ FAIL'}")
        print(f"   💭 Conversation History: {'✅ PASS' if history_ok else '❌ FAIL'}")
        print(
            f"   ⚖️  Comparison: {'✅ PASS' if (comparison_results.get('original') and comparison_results.get('langchain')) else '❌ FAIL'}"
        )
        ...
        if all_passed:
            ...
            print("\n💡 Next steps:")
            print(
                "   1. Test via API: POST /api/v1/chatbot/ with 'use_langchain': true"
            )
